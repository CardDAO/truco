...
contract TrucoChampionsToken is Ownable {
    ...
    modifier onlyMatch(uint256 _tokenId) {
        require(trophies[_tokenId].trucoMatch == msg.sender, "TrucoChampionsToken: Only the match contract can assign tokens");
        _;
    }

    function mint(address _match) public onlyOwner returns (uint256 _tokenId) {
        require(trophiesByMatchAddress[_match].tokenId == 0, "TrucoChampionsToken: Trophy already minted");
    ...
    function assign(address _winner, uint8 _winnerScore, address _loser, uint8 _loserScore, uint256 _trophyId) public onlyMatch(_trophyId) {
    ...
    function trophiesOf(address _player) public view returns (TrucoTrophy[] memory) {
        return trophiesByPlayerAddress[_player];
    }

    function matchesOf(address _player) public view returns (TrucoTrophy[] memory) {
        return matchesByPlayerAddress[_player];
    }

    function getTrophyByMatch(address _match) public view returns (TrucoTrophy memory) {
        return trophiesByMatchAddress[_match];
    }

    function getTrophy(uint256 _tokenId) public view returns (TrucoTrophy memory) {
    ...
    function getPlayers(uint256 _tokenId) public view returns (address, address) {
