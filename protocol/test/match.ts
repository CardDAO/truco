...
describe("Truco Match", function () {
    ...
    async function deployDependencies() {
        ...
        const engine = await TrucoEngine.deploy(trucoin.address);
    ...
    async function deployContract() {
        ...
        const match = await TrucoMatch.deploy(engine.address, deckCrypt.address, trucoin.address, tokenAtStake);

        return { match, engine, trucoin, owner, player2, invalid_player };
    }

    async function startMatch() {
        const { match, trucoin, owner, player2 } = await loadFixture(deployContract);
    ...
    describe ("Join", function () {
        ...
        it("Must not be able to join the match if it is already full", async function () {
            const { match, trucoin, owner, player2, invalid_player } = await loadFixture(deployContract);

            // Allow trucoin transfer
            await trucoin.connect(player2).approve(match.address, tokenAtStake);
            await trucoin.connect(invalid_player).approve(match.address, tokenAtStake);


            await match.connect(player2).join();
            await expect(match.connect(invalid_player).join()).to.be.revertedWith("Match is full");
        ...
        it("Must revert if not enough tokens are approved", async function () {
            ...
            // Allow trucoin transfer
            await trucoin.connect(player2).approve(match.address, tokenAtStake.sub(1));

            await expect(match.connect(player2).join()).to.be.revertedWith("Not enough trucoins transfer approved");
        });
        ...
        it("Owner must not be able to join the match", async function () {
            ...
            await expect(match.connect(owner).join()).to.be.revertedWith("Match creator is already joined");
        });
        ...
        it("Must start the match if both players are joined & tokens are transfered", async function () {
            ...
            expect(await match.connect(player2).join()).to.emit(match, "MatchStarted");
    ...
    describe ("Stake", function () {
        ...
        it("Must emit an event when staking", async function () {
            ...
            await expect(match.connect(owner).stake(0)).to.emit(match, "PlayerStaked");
        });
        ...
        it("Must revert if player is not joined", async function () {
            ...
            await expect(match.connect(player2).stake(0)).to.be.revertedWith("Player is not joined");
        });
        ...
        it("Must revert if player does not have enough tokens approved", async function () {
            ...
            await expect(match.connect(owner).stake(0)).to.be.revertedWith("Not enough trucoins transfer approved");
        });
        ...
        it("Must start the match if both players are joined & tokens are transfered", async function () {
            ...
            expect(await match.connect(owner).stake(0)).to.emit(match, "MatchStarted");
