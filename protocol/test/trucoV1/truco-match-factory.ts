...
describe('Truco Match Factory', function () {
    ...
    describe('New Match', function () {
        it('Should create a new match and emit an Event', async function () {
            const { factory, trucoin, min_bet } = await loadFixture(deployFactoryContract)
            ...
            // Create a new match
            expect(await factory.connect(player1).newMatch(min_bet)).to.emit(factory, "TrucoMatchCreated")
        })
        ...
        it('Should transfer the bet to the match contract', async function () {
            ...
            const event = events.find((e: { event: string }) => e.event === "TrucoMatchCreated")
