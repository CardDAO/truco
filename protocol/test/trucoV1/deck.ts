...
describe("Deck logic", function () {
  ...
  it("Check suits from cards", async function () {
    ...
    expect(await sut.areSameSuit(BigNumber.from(1), BigNumber.from(11))).to.be.false

    // Different suit (not adyacent)
    expect(await sut.areSameSuit(BigNumber.from(1), BigNumber.from(31))).to.be.false

    // Same suit
    expect(await sut.areSameSuit(BigNumber.from(1), BigNumber.from(5))).to.be.true

    // Same suit upper limit
    expect(await sut.areSameSuit(BigNumber.from(1), BigNumber.from(10))).to.be.true

    // Invalid card
    expect(await sut.areSameSuit(BigNumber.from(1), BigNumber.from(41))).to.be.reverted
