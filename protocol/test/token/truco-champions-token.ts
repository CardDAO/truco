...
import { deployTrucoChampionsTokenContract } from '../deploy-contracts'
...
async function assignTokens() {
    ...
    const tx = await trucoChampionsToken.connect(match_factory).mint(match.address)

    const { events } = await tx.wait()

    const event = events?.find(
        (e) => e.event === 'TrucoTrophyMinted'
    )
    ...
    // Assign game result
    await trucoChampionsToken.connect(match).assign(winner.address, winnerScore, loser.address, loserScore, trophy_id)

    return { trucoChampionsToken, match, winner, winnerScore, loser, loserScore, trophy_id }
}
...
describe('Truco Champions Token', function () {
    ...
    describe('Minting', function () {
        ...
        it('Should not allow to mint two tokens to the same match', async function () {
            ...
            await expect(trucoChampionsToken.connect(match_factory).mint(match.address)).to.be.revertedWith('TrucoChampionsToken: Trophy already minted')
        })
    })

    describe('Assign tokens', function () {
        it('Should assign trophy to winner', async function () {
            const {trucoChampionsToken, winner, trophy_id} = await assignTokens()

            // Check if the trophy was assigned to the winner
            expect(await trucoChampionsToken.getWinner(trophy_id)).to.equal(winner.address)
        })
        
        it('Should assign players to trophy', async function () {
            const { trucoChampionsToken, winner, loser, trophy_id } = await assignTokens()
        ...
        it('Should set match to trophy', async function () {
            const { trucoChampionsToken, match, trophy_id } = await assignTokens()

            // Check if the match is the same
            await trucoChampionsToken.getTrophyByMatch(match.address).then((trophy) => {
        ...
        it('Should not allow non-match contract to assign tokens to players', async function () {
            ...
            await expect(
                trucoChampionsToken.connect(non_match).assign(winner.address, 30, loser.address, 20, trophy_id)
            ).to.be.revertedWith('TrucoChampionsToken: Only the match contract can assign tokens')
        })

        // Should show the matches that the player has played
        it('Should show the matches that the player has played', async function () {
            const { trucoChampionsToken, match, winner, loser } = await assignTokens()

            await trucoChampionsToken.matchesOf(loser.address).then((matches) => {
        ...
        it('Should show the matches that the player has won', async function () {
            ...
            await trucoChampionsToken.trophiesOf(winner.address).then((matches) => {
                expect(matches[0].trucoMatch).to.equal(match.address)
            })
        })
