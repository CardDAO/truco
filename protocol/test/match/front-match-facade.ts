...
describe('Front Match Facade', function () {
    ...
    async function deployContract() {
        ...
        return { frontMatchFacade, match, engine, trucoin, player1, player2, invalid_player }
    }
    ...
    describe('Can\'t spell', function() {
        it('Player1 should not be able spell Envido', async function () {
            ...
            expect(await frontMatchFacade.connect(player1).canSpellEnvido(match.address)).to.be.equal(false)
        })

        it('Invalid player should not be able spell Envido', async function () {
            const { frontMatchFacade, match, invalid_player } = await loadFixture(
                deployContract
            )

            // invalid player can't spell envido 
            await expect(frontMatchFacade.connect(invalid_player)
        ...
        it('Player1 should not be able to spell Truco', async function () {
            ...
            expect(await frontMatchFacade.connect(player1).canSpellTruco(match.address)).to.be.equal(false)
        })
    ...
    describe('Can spell', function () {
        ...
        it('Envido -> player2', async function () {
            ...
            expect(await frontMatchFacade.connect(player2).canSpellEnvido(match.address)).to.be.equal(true)
        })
        ...
        it('Truco -> player2', async function () {
            ...
            expect(await frontMatchFacade.connect(player2).canSpellTruco(match.address)).to.be.equal(true)
        })
        ...
        it('Player1 should be able to send response when challange is Envido', async function () {
            ...
            expect(await frontMatchFacade.connect(player1).canResponse(match.address)).to.be.equal(true)
        })
    ...
    describe('Check EnvidoPoints', function() {
        it('How much quantity do i have for spell envido with all my cards', async function () {
            ...
            expect(await frontMatchFacade.getEnvidoPointsForCards(cards)).to.be.equal(5)
        })
    })
