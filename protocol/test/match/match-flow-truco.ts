...
describe('Multi Transaction Test: Truco', function () {
    ...
    describe('Turn Handling', function () {
        ...
        it('Playing card out of turn: after round 1', async function () {
            ...
            expect(match.connect(player2).playCard(BigNumber.from(5))).to.be
                .reverted
        })
        ...
        it('Playing card out of turn: after round 2', async function () {
            ...
            expect(match.connect(player1).playCard(BigNumber.from(7))).to.be
                .reverted
        })
        ...
        it('No truco spelled, but playing cards, draw on first round', async function () {
            ...
            expect(match.connect(player1).playCard(BigNumber.from(14))).to.be
                .reverted
        })
        ...
        it('No truco spelled, draw on 1st and 2nd round and 3rd round, mano should win', async function () {
            ...
            // Round 1
            await expect(match.connect(player2).playCard(BigNumber.from(2))).to.emit(
                match,
                'TurnSwitch'
            ).withArgs(player1.address) // 2 of Coins
            await expect(match.connect(player1).playCard(BigNumber.from(22))).to.emit(
                match,
                'TurnSwitch'
            ...
            await expect(await match.connect(player1).playCard(BigNumber.from(14))).to.not.emit(
                match,
                'TurnSwitch'
            ) // 2 of Swords  (turn shouldn't switch because it's final)
        ...
        it('Spelling Truco after an Envido refused, and spell it again', async function () {
            ...
            await expect(match.connect(player2).spellTruco()).to.emit(
                match,
                'TurnSwitch'
            ).withArgs(player1.address)

            // Spell envido and check turn event emitted
            await expect(match.connect(player1).spellEnvido()).to.emit(
                match,
                'TurnSwitch'
